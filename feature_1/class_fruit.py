...
# input 으로 /fruit_shop 내부의 폴더 를 지정해주면 내부의 파일들을 읽어서 리스트로 변환
    def file_finder(self, folder_name):
        ...
        import os
        files = os.scandir("D:/leety/Desktop/fruit_shop/%s" % folder_name)
        for file in files:
...
# 읽어온 파일들의 첫째줄, 둘째줄을 가져옴
    def file_reader(self):
        for i in self.files:
            with open("D:/leety/Desktop/fruit_shop/{0}/{1}".format(self.folder_name, i), 'r') as f:
    ...
    def writing(self):
        with open("D:/leety/Desktop/fruit_shop/{0}".format("output sales"),'w') as f:
